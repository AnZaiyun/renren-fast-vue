 ...
 * 父子组件传递数据
 * 1）、子组件给父组件传递，使用事件机制
 *      子组件给父组件发送事件，事件携带数据，使用this.$emit("tree-node-click",data, node, component)
 *      第一个参数为传递的事件名，后面为任意多的数据，可用来传递参数
 * 2）、父组件接收事件，并绑定自己的时间<category @tree-node-click="treeNodeClick"></category>
 * 
 */
